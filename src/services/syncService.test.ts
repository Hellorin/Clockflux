...
const emptyData = { days: {}, daysOff: {}, settings: { annualHolidayAllowance: 25, employmentStartDate: null, holidayAccrualMode: 'gradual' as const, themeLightColor: null, themeDarkColor: null, dailyTargetHours: 8 } }
...
describe('pushSync', () => {
  ...
  it('PUTs the data and returns the parsed response on success', async () => {
    const body = { lastSyncedAt: '2026-08-11T10:00:00Z' }
    vi.mocked(fetch).mockResolvedValue(new Response(JSON.stringify(body), { status: 200 }))

    const result = await pushSync('access-token-123', emptyData)
